...
def train_flow_matching_noprop(model, data_loader, val_loader, criterion, epochs, device, lr, lr_backbone, trial=None): # MODIFICATION 1: Add trial=None
    param_dicts = [
    ...
    for epoch in range(epochs):
        ...
        avg_epoch_loss = epoch_loss_sum / model.T if model.T > 0 else 0
        ...
        # MODIFICATION 2: Add this block for Optuna pruning
        if trial:
...
def objective(trial):
    ...
    t_steps = trial.suggest_int("t_steps", 2, 8) # This is num_decoder_layers
    batch_size = trial.suggest_categorical("batch_size", [4, 8, 12])

    # Static Hyperparameters
    D_MODEL, NUM_QUERIES, NUM_CLASSES = 128, 20, 20
    EPOCHS_FOR_SEARCH = 15 # Shorter search time

    print(f"\n--- Starting Trial {trial.number} ---")
    print(f"Params: lr={lr:.6f}, lr_backbone={lr_backbone:.6f}, n_head={n_head}, t_steps={t_steps}, batch_size={batch_size}")
    ...
    try:
        ...
        # Use suggested hyperparameters to create the model
        model = FlowMatchingNoPropDetector(NUM_CLASSES, NUM_QUERIES, D_MODEL, n_head, t_steps)
        for block in model.vector_field_blocks:
            block.num_queries = NUM_QUERIES

        # Run the training, passing the trial object
        history = train_flow_matching_noprop(model, data_loader, val_loader, criterion, EPOCHS_FOR_SEARCH, device, lr=lr, lr_backbone=lr_backbone, trial=trial)
