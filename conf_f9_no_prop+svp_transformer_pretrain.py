...
class ViTLocal(nn.Module):
    def __init__(self, model_name='vit_tiny_patch16_224', proj_dim=10, pretrained_path=None):
        super().__init__()
        if pretrained_path and os.path.exists(pretrained_path):
            base = timm.create_model(model_name, pretrained=False)
            
            # Store original weights for comparison
            original_weight = base.patch_embed.proj.weight.clone()
            
            # Load pretrained
            state_dict = load_file(pretrained_path)
            new_state_dict = {k.replace('module.', ''): v for k, v in state_dict.items()}
            missing_keys, unexpected_keys = base.load_state_dict(new_state_dict, strict=False)
            
            # Check if weights actually changed
            if torch.equal(original_weight, base.patch_embed.proj.weight):
                print("WARNING: Pretrained weights were not loaded - using timm pretrained instead")
        ...
        self.random_projs = nn.ParameterList([
            nn.Parameter(torch.randn(proj_dim, base.embed_dim), requires_grad=False)
    ...
    def forward(self, x, y,target_block_idx=None):
        ...
        for i, block in enumerate(self.blocks):
            if i < target_block_idx:
                ...
                v = self.dropout(F.linear(x, self.random_projs[i]))
            ...
            x = F.layer_norm(x, x.shape[-1:])
            v_prev = self.dropout(F.linear(x, self.random_projs[i])).detach()
...
class NoPropDT(nn.Module):
    def __init__(self, embed_dim=192, num_classes=10, num_blocks=10, eta=1.0):
        ...
        self.vit_local = ViTLocal(model_name="vit_tiny_patch16_224", proj_dim=num_classes, pretrained_path='None')
    ...
    def predict(self, x):
        ...
        h, _, _ = self.vit_local(x, dummy_labels, target_block_idx=6)  # ignore local losses
...
def visualize_label_denoising(model, num_samples=200):
    ...
    for inputs, labels in train_loader:
        ...
        u_y = model.embed(labels[:num_samples])  # clean label embeddings
        break  # just take 1 batch
    ...
    for t in range(1, num_blocks + 1):
        ...
        z_ts.append(model.z_proj(z_t).cpu())  # now shape = [B, 256]
        u_t_hats.append(u_t_hat.cpu())        # also shape = [B, 256]
        ts.append(t)

    # Flatten: [T x B x D] → [B*T x D]
    ...
    plt.show()
for epoch in range(100):  # Short for debugging
    model.train()
    total_loss = 0
    for x, y in tqdm(train_loader, desc=f"Epoch {epoch+1}"):
        ...
        max_t_vit = max(0, min(max_t_vit, 6))  # Clamp to [0, 6]
        t_vit = random.randint(0, max_t_vit)   # NOTE: 0-indexed

        # === Denoise: Curriculum scheduling for 10 blocks ===
        denoise_progress = epoch / 100
        max_t_denoise = int(denoise_progress * model.num_blocks)
        max_t_denoise = max(1, min(max_t_denoise, model.num_blocks))  # Clamp to [1, 10]
