...
# --- ORIGINAL NOPROP COMPONENTS (Kept for reference) ---
...
def train_standard_end_to_end(model, data_loader, val_loader, criterion, epochs, device, lr, lr_backbone):
    param_dicts = [
    ...
    for epoch in range(epochs):
        model.train()
        print(f"--- Standard End-to-End Epoch {epoch+1}/{epochs} ---")
        epoch_loss_sum = 0
        num_batches = 0
        ...
        for images, targets in progress_bar:
            ...
            # Standard backpropagation and optimization
            optimizer.zero_grad()
            total_loss.backward()
...
if __name__ == '__main__':
    # Hyperparameters
    D_MODEL, N_HEAD, NUM_QUERIES, NUM_CLASSES, T_STEPS, EPOCHS, BATCH_SIZE = 128, 8, 20, 20, 24, 50, 8 
    LR = 1e-4
    LR_BACKBONE = 1e-5
    ...
    # To run the new standard end-to-end model, use this block:
    print("\n" + "="*50 + "\n     RUNNING STANDARD END-TO-END MODEL     \n" + "="*50)
    standard_model = StandardEndToEndDetector(NUM_CLASSES, NUM_QUERIES, D_MODEL, N_HEAD, T_STEPS)
    
    if device.type == 'cuda':
        torch.cuda.reset_peak_memory_stats(device)

    start_time = time.time()
    history = train_standard_end_to_end(standard_model, data_loader, val_loader, criterion, EPOCHS, device, lr=LR, lr_backbone=LR_BACKBONE)
    
    training_time = time.time() - start_time
    peak_mem = torch.cuda.max_memory_allocated(device) / 1e9 if device.type == 'cuda' else 0

    MODEL_SAVE_PATH = "standard_end_to_end_resnet50.pth"
    RESULTS_SAVE_PATH_PNG = "standard_end_to_end_results.png"
    RESULTS_SAVE_PATH_PT = "standard_end_to_end_results.pt"
    
    torch.save(standard_model.state_dict(), MODEL_SAVE_PATH)
    print(f"\nModel successfully saved to {MODEL_SAVE_PATH}")
    
    final_map = history[-1]['mAP']
    
    # ... The rest of the main block for plotting and saving results ...
    print("\n" + "="*50 + "\n     Standard End-to-End (ResNet-50) RESULTS     \n" + "="*50)
    ...
    plt.subplot(1, 2, 1)
    plt.plot([h['epoch'] for h in history], [h['mAP'] for h in history], marker='o', label='Standard mAP', linewidth=2, color='green')
    plt.title('Standard Model Training Progress')
    plt.xlabel('Epoch')
    plt.ylabel('Validation mAP')
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.subplot(1, 2, 2)
    plt.plot([h['epoch'] for h in history], [h['loss'] for h in history], marker='s', label='Training Loss', linewidth=2, color='orange')
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(RESULTS_SAVE_PATH_PNG)

    results = {
        'method': 'Standard End-to-End (ResNet-50)',
        'final_map': final_map,
        'training_time': training_time,
        'peak_memory': peak_mem,
        'history': history
    }
    torch.save(results, RESULTS_SAVE_PATH_PT)
    print(f"\nResults plot saved to '{RESULTS_SAVE_PATH_PNG}'")
    print(f"Results dictionary saved to '{RESULTS_SAVE_PATH_PT}'")
